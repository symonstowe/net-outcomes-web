...
function readShareStateFromUrl() {
  ...
  return {
    ...
    goalieTeam: normalizeTextValue(params.get('goalieTeam')),
...
function syncUrlState() {
  ...
  if (activeSection === 'rankingsPanel') {
    ...
    const goalieTeam = normalizeTextValue(document.getElementById('goalieTeamSearch')?.value);
...
function renderTeamRankings(rows) {
  ...
  tbody.innerHTML = rows.map((row, idx) => `
    <tr>
      ...
      <td class="${classForSigned(row.chance_suppression)}">${signed(row.chance_suppression)}</td>
      <td class="${classForSigned(row.offensive_depth)}">${signed(row.offensive_depth)}</td>
      <td class="${classForSigned(row.defensive_depth)}">${signed(row.defensive_depth)}</td>
...
function depthValue(depthState, key) {
  if (!depthState || typeof depthState !== 'object') return 0;
  const rawValue = Number(depthState?.[`${key}_raw`]);
  if (Number.isFinite(rawValue)) return rawValue;
  const fallbackValue = Number(depthState?.[key]);
  return Number.isFinite(fallbackValue) ? fallbackValue : 0;
}

function renderDepthChip(label, currentValue, suggestedValue) {
  const delta = Number(suggestedValue || 0) - Number(currentValue || 0);
  const deltaClass = classForSigned(delta);
  return `
    <span class="sf-score-chip sf-score-chip--depth ${deltaClass}">
      <span class="sf-score-chip-label">${esc(label)}</span>
      <span class="sf-score-chip-meta">${signed(currentValue)} -> ${signed(suggestedValue)} (${signed(delta)})</span>
    </span>
  `;
}
...
function renderTeamCard(team) {
  ...
  const currentDepth = team.depth?.current || {};
  const suggestedDepth = team.depth?.suggested || {};
  ...
  return `
    <article class="sf-team-card" data-team="${esc(team.team)}">
      ...
      <div class="sf-score-compare sf-score-compare--depth">
        ${renderDepthChip('Off Depth', depthValue(currentDepth, 'offence'), depthValue(suggestedDepth, 'offence'))}
        ${renderDepthChip('Def Depth', depthValue(currentDepth, 'defence'), depthValue(suggestedDepth, 'defence'))}
      </div>
      <div class="sf-team-depth-note">Depth rewards teams that can still drive play beyond the top line or pair and that can ice credible second PP and PK groups.</div>
...
function sortTeamRankingsRows(rows, sortState) {
  ...
  const fieldMap = {
    ...
    chance_suppression: { field: 'chance_suppression', type: 'number' },
    offensive_depth: { field: 'offensive_depth', type: 'number' },
    defensive_depth: { field: 'defensive_depth', type: 'number' },
...
function applyGoalieFilter(allRows, query, teamQuery) {
  const q = String(query || '').toLowerCase().trim();
  const teamQ = String(teamQuery || '').toLowerCase().trim();
  return allRows.filter((row) => {
    const goalieMatch = !q || String(row.goalie_name || '').toLowerCase().includes(q);
    const teamMatch = !teamQ || String(row.team || '').toLowerCase().includes(teamQ);
...
async function main() {
  ...
  const goalieTeamSearch = document.getElementById('goalieTeamSearch');
  setControlValue(goalieSearch, state.initialUrlState.goalie);
  setControlValue(goalieTeamSearch, state.initialUrlState.goalieTeam);
  const refreshGoalies = () => {
    const rows = applyGoalieFilter(
      allGoalies,
      goalieSearch?.value || '',
      goalieTeamSearch?.value || ''
    );
    renderGoalies(rows);
    syncUrlState();
  };
  goalieSearch.addEventListener('input', refreshGoalies);
  goalieTeamSearch.addEventListener('input', refreshGoalies);
